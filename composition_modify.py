...
sys.path.append("model/ControlCom_Image_Composition")
import cv2
import torch
...
print(sys.path.insert(0, proj_dir))
...
def argument_parse():
    ...
    parser.add_argument(
        ...
        default="/home/tailydoodly/model/2_composition_input"
    )
    parser.add_argument(
        "--outdir",
        type=str,
        ...
        default="/home/tailydoodly/model/3_composition_output"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_false',
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./model/ControlCom-Image-Composition/configs/controlcom.yaml",
        help="path to config which constructs model",
    )
    parser.add_argument(
        "--ckpt_dir",
        default="model/ControlCom-Image-Composition/checkpoints",
...
if __name__ == "__main__":
    ...
    fg_name = os.listdir(os.path.join(opt.testdir, 'foreground'))
    fg_name = fg_name[0]
    fg_path = os.path.join(opt.testdir, 'foreground', fg_name)
    fgmask_name = os.listdir(os.path.join(opt.testdir, 'foreground_mask'))
    fgmask_name = fgmask_name[0]
    fgmask_path = os.path.join(opt.testdir, 'foreground_mask', fgmask_name)
